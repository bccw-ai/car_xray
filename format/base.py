...
class Base(object):
  def __init__(self, data, headers, keys, addr_blocks, encrypted):
    ...
    # <: little-endian; L: C Type = unsigned long python = long
    # get data[-4] = 4bytes in 16bytes, e.g.:file checksum: 0x4b549eb
    self._file_checksum      = struct.unpack('<L', data[-4:])[0]  
    self._firmware_blocks    = addr_blocks
    self._firmware_encrypted = encrypted  # link x5a or x31; self.firmware_encrypted()
    self._keys               = keys
  ...
  # key form firmware.header, e.g. "5 = ['\x0f\x10\x0e']"
  def _get_decoder(self, key1, key2, key3, op1, op2, op3):
    decoder = {}
    values = set()  # 创建一个无序不重复的元素集

    for e in range(256):
      # input ascii -> cipher -> output new ascii
      d = op3(op2(op1(e, key1), key2), key3) & 0xFF
      decoder[chr(e)] = chr(d)  # value: key
      values.add(d) 
  ...
  def decrypt(self, search_value):
    ...
    keys = list()
    # keys = [{'sym': 'k0', 'val': 19}, {'sym': 'k1', 'val': 16}, {'sym': 'k2', 'val': 15}]
    ...
    for k1, k2, k3 in key_perms:
      for o1, o2, o3 in op_perms:
        ...
        # 在decoder中找到合适的对应关系，将fw中的解密成正常的字符形式用于逆向 
    ...
    print("")
    print("display_ciphers = {}".format(display_ciphers)) 
  ...
  def __str__(self):
    info = [ 
      "file format: {}".format(self._file_format),
      "file format: {}".format(hex(self._file_checksum)),
    ]
    info.append("headers:")
